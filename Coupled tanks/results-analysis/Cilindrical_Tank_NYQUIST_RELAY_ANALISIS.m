...
figure(1);
hold on
plot(u,'b'); %Gera o grÃ¡fico Tempo x SaÃ­da
plot(y,'r'); %Gera o grÃ¡fico Tempo x SaÃ­da
hold off;
saidas=[Tempo; u; y]';
%%
...
a
%%
%logo apos analise dos graficos calculamos os valores da funções de
%transferencia
...
Ncrt = (4*d)*((sqrt(a.^2 - ep2.^2)./a) + (1i.*ep2./a))./(pi.*a); % estimativa do ganho crítico.
for i = 1:length(Ncrt)
    num = [-1];
    den = [Ncrt(i)];
    GF(i) = tf(num,den);%funções de transferencia estimadas
end
...
% nyquist(GZ); title('Comparação dos diagramas');
 hold on;
