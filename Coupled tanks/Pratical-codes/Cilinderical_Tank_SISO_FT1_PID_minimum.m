...
%% 
     for k=5:npts
        ...
        rate(k)=(erro(k) - erro(k-1));%/Tc; %Rate of erro
         ...
         mi1 = (rate(k)/(2*L) + 1/2)*(erro(k)/(2*L) + 1/2); mi2 = (erro(k)/(2*L) + 1/2)*(-rate(k)/(2*L) + 1/2); mi3 = (-erro(k)/(2*L) + 1/2)*(rate(k)/(2*L) + 1/2); mi4 = (-erro(k)/(2*L) + 1/2)*(-rate(k)/(2*L) + 1/2); 
            
         Am(k) = mi1*H1 + mi2*H2 + mi3*H3 + mi4*H4;
